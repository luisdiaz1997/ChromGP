#!/bin/bash
# Run MGGP SVGP pipeline for assigned chromosomes on a specific GPU.
# Usage: run_mggp_svgp_chromosomes.sh <GPU_ID> <START_CHR> <END_CHR>
#
# Example:
#   run_mggp_svgp_chromosomes.sh 0 1 11   # chr1-11 on GPU 0
#   run_mggp_svgp_chromosomes.sh 1 12 22  # chr12-22 on GPU 1
...
echo "======== GPU $GPU_ID: chr${START} through chr${END} (MGGP SVGP) ========"
...
for chr in $(seq "$START" "$END"); do
    ...
    echo "============================================================"
    echo "  GPU $GPU_ID — chr$chr  (MGGP SVGP)"
    echo "  Config: $CONFIG"
    echo "  Start: $(date)"
    echo "============================================================"
